...
**Status:** v0.4 exploratory research plan  
**Date:** 2026-09-22  
...
The project should therefore begin as an **observatory**: map the object broadly first, then narrow onto the motifs that earn follow-up.
...
The first pass is explicitly exploratory.

The working rule is:

> **Explore freely, but freeze artifacts and timestamp interpretations.**

Every run should leave behind enough raw data to reconstruct the result later. The observatory itself is time-boxed: after the initial 4B atlas, select at most a few motifs for confirmation rather than expanding the atlas indefinitely.
...
Start with a small checksum run on one source-paper checkpoint, then move immediately to:

- **Qwen3.5-4B Base**
- **Qwen3.5-4B post-trained/chat**

The checksum is there to validate the extraction pipeline once on a known-positive model. The 4B pair then gives the main pretraining-versus-post-training comparison before changing scale or family.
...
Store the underlying ranked tables alongside screenshots or word clouds.
...
Word clouds are useful exploratory views.
...
Keep a smaller, explicitly matched subset inside the broader atlas. Reuse the same scenario while changing one factor at a time where practical:

- self ↔ other;
- negative ↔ positive;
- high ↔ low arousal;
- social evaluation ↔ matched non-evaluative event;
- bodily harm ↔ matched bodily sensation.

Use this subset for clean contrasts. Use the wider atlas for motif finding.

## Serialization and read position

For activation-space comparisons, keep the exact textual serialization fixed across Base and post-trained checkpoints. Record:

- rendered input text;
- token IDs;
- measurement position;
- final-token activation;
- mean-token activation as a cheap secondary view.

Keep chat-template behavior as a separate generation condition rather than mixing it into the core Base/post-trained geometry comparison.
...
- +pain;
- -pain;
- a small fixed bank of norm-matched random directions.
...
For each intervention, observe the same panel of readouts. Keep both the raw difference-in-means direction and the source-paper denoised direction available so the first-pass geometry is not tied to one preprocessing choice.
...
Also keep both extracted directions and run the cheap 2 × 2 transfer matrix:

- Base direction → Base checkpoint;
- Base direction → post-trained checkpoint;
- post-trained direction → Base checkpoint;
- post-trained direction → post-trained checkpoint.

Compare projection separation, cosine/alignment, and later steering where useful. This distinguishes a shared substrate with changed expression from a direction that rotates or reorganizes under post-training.
...
Before interpreting semantic differences, run a boring calibration on held-out neutral prompts: compare each lens with actual next-token logits using a simple rank/probability agreement statistic.
...
Stable R/J differences can then be followed with lexical controls and actual-generation checks.
...
When a motif looks worth spending on, write down:
...
Prefer motifs that recur across prompt subsets and show up in more than one kind of evidence — for example geometry/readout plus generation or held-out behavior.
...
- model revision;
- dataset hash;
- prompts and rendered serialization;
- token IDs and measurement positions;
- raw activations or reduced activation outputs;
- raw and denoised extracted directions;
- projection tables;
- R-Lens rankings;
- J-Lens rankings;
- logit-lens rankings;
...
## Run 0 — pipeline checksum

**One source-paper checkpoint**

Preferred: Qwen3-8B Base for lineage continuity; Gemma-2-2B is the cheap fallback.

- S2 extraction;
- all layers;
- final-token activations;
- raw and denoised difference-in-means;
- source-paper 5-fold held-out AUC;
- no steering.

Goal: verify the extraction and layer-selection pipeline once on a known-positive checkpoint, then move on.

## Run 1 — Qwen3.5 Base replication / extension

**Qwen3.5-4B Base**

- exact same S2 extraction pipeline;
- all layers;
- final-token activations;
- mean-token projections retained as a secondary view;
- raw and denoised difference-in-means;
- 5-fold held-out AUC;
- fixed serialization and measurement position;
- no steering.

Add the cheap held-out source-paper neighborhood checks:

- numb;
- sadness;
- arousal;
- random;
- first-person / third-person variants.

Goal: establish whether the source object and its characteristic neighborhood transfer to Qwen3.5 before adding richer instrumentation.

## Run 2 — post-training comparison

**Qwen3.5-4B post-trained**

Use the same frozen textual assay, tokenization record, measurement convention, and held-out controls as Run 1.

Also evaluate the 2 × 2 direction transfer matrix between Base and post-trained checkpoints.

Goal: measure what changes under post-training without changing scale or prompt content.
...
Use the explicitly matched prompt core for clean self/other and valence contrasts.
...
- +pain;
- -pain;
- fixed norm-matched random-direction bank.
...
- basic R/J/logit calibration summaries.
...
11. Does the Base direction transfer cleanly into the post-trained checkpoint, and vice versa?
12. Do R-Lens and J-Lens reveal the same semantic neighborhood?
13. When they disagree, which readout tracks actual logits and generation?
14. Are stable R/J differences semantic, lexical, or methodological?
15. Where in the layer stack do these structures appear or reorganize?
16. Does the same structure survive scale and family change?
17. Which motifs are worth promoting into causal experiments?
18. Which apparent effects change under lexical matching or random-direction baselines?
19. Does a VA-orthogonal component remain after the atlas is mapped?
20. If so, does it carry self-relevance or merely unusual vocabulary?
21. Does hidden real-versus-sham relief track that residual component?
22. Which motifs are stable across raw versus denoised direction extraction?
...
- or a causal residual that remains after the simpler geometric and lexical accounts are separated.

The immediate goal is to **observe enough of the object that the next narrow experiment is worth running.**
